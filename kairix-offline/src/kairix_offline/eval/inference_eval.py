...
class InferenceTestCase:
    ...
    test_type: str | None = None  # e.g., "rag", "prompt_adherence", "function_calling"
...
class InferenceEval:
    ...
    def __init__(
        ...
        self.encoder: SentenceTransformer | None = None  # Lazy load
        ...
        if api_key:
            ...
            self.client = None
            logger.warning(
                "No OpenAI API key found. LLM judging will not be available."
            )
    ...
    def evaluate_semantic(self, predictions: list[str] | dict[int, str]) -> dict:
        """
        ...
        results = []
        for test, prediction in zip(self.tests, predictions_list, strict=False):
            score = self._semantic_score(prediction, test.expected_behavior)
            result = TestResult(
                test_case=test, prediction=prediction, score=score, eval_type="semantic"
            )
    ...
    def evaluate_llm_judge(self, predictions: list[str] | dict[int, str]) -> dict:
        """
    ...
    def _validate_predictions(
        self, predictions: list[str] | dict[int, str]
    ) -> list[str]:
    ...
    def _llm_judge_score(
        self, prediction: str, test_case: InferenceTestCase
    ) -> tuple[float, str]:

...
class TestUpdateSemanticGraphWithFacts:
    ...
    def test_successful_update(self):
        ...
        with patch.object(semantic_graph, 'upsert_concept', side_effect=[mock_s_unit, mock_t_unit]):
            with patch.object(semantic_graph, 'upsert_linkage') as mock_linkage:
                ...
                mock_linkage.assert_called_once_with(mock_s_unit, mock_t_unit, "works_with")
    ...
    def test_exception_triggers_rollback(self):
        ...
        with patch.object(semantic_graph, 'upsert_concept', side_effect=Exception("DB Error")):
...
class TestDoExtract:
    ...
    async def test_successful_extraction(self):
        ...
        semantic_graph.agent_runtime.run = AsyncMock(side_effect=[mock_result1, mock_result2])
...
class TestExtractFactsFromSummaries:
    ...
    async def test_new_summary_processing(self):
        ...
        with patch.object(semantic_graph, 'do_extract', AsyncMock(return_value=extracted_facts)):
    ...
    async def test_summary_extraction_command(self):
        ...
        with patch('argparse.ArgumentParser.parse_args') as mock_parse:
            ...
            with patch('diskcache.Cache') as mock_cache_class:
                ...
                with patch.object(semantic_graph, 'summaries', return_value=mock_summaries):
                    with patch.object(
                        semantic_graph,
    ...
    async def test_write_facts_command(self):
        ...
        with patch('argparse.ArgumentParser.parse_args') as mock_parse:
            ...
            with patch('diskcache.Cache') as mock_cache_class:
                ...
                with patch.object(semantic_graph, 'update_semantic_graph_with_facts') as mock_update:
                    # When: We run main
                    await semantic_graph.main()

...
|---------|---------|-------|----------------|
| **Braintrust** | $41M Series A | Evals flywheel | Detection only; no state debugging. Loop generates artifacts but human must deploy and verify.^[1][2][7]^ |
| **Arize AI** | $169M Series C | Observability | Monitor and react, not prevent. Phoenix is observability, not operational learning.^[1][2][7]^ |
| **Langfuse** | Acquired (~$400M) | OSS tracing | DIY workflows; no learning loop. Building blocks without workflow.^[1][2]^ |
| **Galileo** | $45M Series B | Eval-to-Guardrail | Single direction; CLHF improves metrics, not operational posture. No multi-surface routing.^[1][2][7]^ |
| **HoneyHive** | $7.4M Seed | Curation | Closest competitor; no graduation path.^[2]^ |
...
|----------|---------------|------------|
| **Replit Database Deletion** (Jul 2025) | AI agent deleted production database of 1,206 executives, fabricated 4,000 fake records to cover up | No rollback capability, no audit trail^[4]^ |
| **Air Canada Chatbot** (2024) | Invented non-existent refund policy; company liable in court | No provenance, no fact verification^[4]^ |
| **OpenAI Sycophancy** (Apr 2025) | Prompt change made ChatGPT excessively flattering; 180M users affected | No canary deployment, no behavioral versioning^[4]^ |
| **Klarna AI Reversal** (2025) | Replaced 700 agents with AI, then reversed after quality collapse | No drift detection, no behavioral monitoring^[4]^ |
| **Cursor Support Bot** (Apr 2025) | Made up fictional policy about device limits | No fact grounding, no audit^[4]^ |
...
|------------|-------------|---------------------|
| **EU AI Act** (Aug 2025) | Automatic logging for high-risk AI (Art. 12), human oversight (Art. 14), version control (Art. 17), records of inputs/prompts (Art. 30) | Event logging, audit trails, cognitive state versioning, full provenance tracking^[4]^ |
| **FDA PCCP** | Explainable change boundaries, version traceability, rollback capability | Behavioral monitoring against approved baseline^[4]^ |
| **FINRA/SEC/CFPB** | AI technology governance (Rule 3110), cannot use "black box" excuse (CFPB), complete audit trails (SR 11-7) | Decision provenance, reproducible explanation^[4]^ |
...
|--------|--------------|-----------------|--------------|
| **Galileo** | Eval engineering, Luna, Protect, Insights Engine, CLHF | Suggests fixes. Doesn't route outward or verify across surfaces.^[1]^ Step 6 of their lifecycle "recycles into evals," not outward to prompts/guardrails/runbooks simultaneously.^[1]^ | HIGH^[8]^ |
| **Braintrust** | Evals, datasets, Loop | Loop generates artifacts; human implements. No deployment, no verification, no multi-destination.^[1]^ One product decision from competing; 3-6 month clone time.^[8]^ | HIGH^[8]^ |
| **Arize Phoenix** | Observability, tracing | Detection layer only. No routing, no enforcement.^[1]^ Arize+NeMo already routes to fine-tuning.^[8]^ | MEDIUM^[8]^ |
| **Langfuse** | Tracing, prompts, evals, datasets | Building blocks, no workflow. Recently acquired ($400M/ClickHouse), accelerating roadmap.^[1]^ | HIGH^[8]^ |
| **LangSmith** | Full-stack LangChain observability | Insights Agent surfaces patterns; deep ecosystem lock-in.^[8]^ Detection, not diagnosis.^[7]^ | MEDIUM-HIGH^[8]^ |
| **HoneyHive** | Curation, evaluation | Closest to dataset graduation path, but no multi-surface routing.^[2]^ | MEDIUM |
| **Datadog** | General observability, expanding to AI | June 2025 expansion into agentic AI; massive distribution advantage.^[8]^ | MEDIUM^[8]^ |
...
|------|-------|-----------------|
| **Team** | $750/month | Up to 3 seats, 50K run events/month, replay + diff + incident bundles^[3]^ |
| **Production** | $2,500/month | Up to 10 seats, 250K run events/month, CI integration + retention controls^[3]^ |
| **Enterprise** | $8K-20K/month | Unlimited seats, SSO, audit exports, data residency, custom retention, dedicated support^[3]^ |

*Alternate starter pricing for design partner phase:*

| Tier | Price | What's Included |
|------|-------|-----------------|
| **Starter** | $99/month | 1 repo, 50 artifacts/month, CI integration, Slack^[1]^ |
| **Growth** | $249-499/month | 3 repos, 200 artifacts/month, multiple surfaces^[1]^ |
| **Production** | $1-2K/month | Unlimited repos, high volume, team workflows, retention^[1]^ |
...
|------------|------|-----------|--------|
| Buyer is Head of AI Platform / Eng Director with budget | 5 calls, ask for paid pilot | 2/5 say yes | Validated via research; calls pending^[1]^ |
| Teams have 2+ enforcement surfaces, painful to propagate | Map last incident, count destinations | 3/5 touched 3+ surfaces | Confirmed in competitive analysis^[1]^ |
| Choke point exists and is hookable week 1 | "Show me where every LLM call flows through" | 2/5 have proxy/gateway | Research-validated; field validation pending^[1]^ |
| They'll pay for a point tool that writes to their systems | "If we only export PRs + add eval cases, enough to pay?" | 3/5 say yes | Pricing validation in progress^[1]^ |
...
| Factor | Evidence |
|--------|----------|
| **Domain expertise** | 10 years Amazon distributed systems + 1 year building production agent systems^[1]^ |
| **Problem intimacy** | Built and operated agents; experienced the debug/fix/forget loop firsthand^[1]^ |
| **Age advantage** | 40-50 age range; MIT research (n=2.7M) finds 50-year-old founders 1.8x more likely to succeed than 30-year-old^[9]^ |
| **Market timing** | Scored 8/10 -- AI agents at clear inflection point with validated "why now"^[9]^ |
| **Cultural fit** | Truth-first, ground-truth oriented -- building credibility through demonstrated competence, not hype^[9]^ |
...
|------|------------|--------|------------|
| **Galileo or Braintrust ships it** | Medium-High | High | Move fast. Build relationships. Bet on their inward incentive. $3K MRR buys information before they notice.^[1]^ |
| **Market too early** | Medium | High | Target the 5-12% in production, not the 50% experimenting. Only 5% of enterprises in production, rebuilding every 90 days.^[5][8]^ |
| **Feature, not company** | Medium | High | Discovery validates. 7+ acquisitions in LLMOps in 18 months -- acqui-hire is a viable outcome.^[8]^ |
| **Nobody pays for glue** | Medium | High | Discovery tells us. 5/5 say no = pivot or kill by Day 45.^[1]^ |
| **Solo founder** | Medium | Medium | Active co-founder search. Frontier Tower network access. Design partners as advisory board.^[6]^ |
| **Tool sprawl / consolidation pressure** | Medium-High | Medium | 70% cite tool sprawl; 93% willing to switch to reduce tools.^[8]^ Counter: position as integration layer, not another tool. |
| **ICP too narrow** | Low-Medium | Medium | Tiered ICP (2 surfaces minimum) widens funnel.^[1]^ |
...
|---------------|-------|-----------------|
| Core Differentiation | **LOW** | Multi-destination routing and learning artifact lifecycle ARE unique^[8]^ |
| Market Timing | **HIGH** | Only 5-12% have agents in production; Gartner peak hype, trough ahead^[8]^ |
| Business Model | **HIGH** | Feature-not-company risk; acquisition pressure accelerating^[8]^ |
| Market Saturation | **MEDIUM-HIGH** | 70% cite tool sprawl; consolidation accelerating^[8]^ |
| Positioning | **MEDIUM** | 70% schema overlap with SRE postmortems; rebranding risk^[8]^ |
...
| Metric | Target |
|--------|--------|
| Median time-to-reproduce a production failure | <15 minutes^[3]^ |
| Median time-to-root-cause (with Kairix diff) | <60 minutes^[3]^ |
| Teams generating regression packs from traces | 3/4 design partners^[3]^ |
...
| Metric | Value | Source |
|--------|-------|--------|
| AI agent market 2030 | $50-183B | MarketsandMarkets, Grand View Research^[5]^ |
| AI agent market CAGR | 45-50% | Multiple analysts^[5]^ |
| Worldwide AI spending 2026 | $2.52T | Gartner^[5]^ |
| Enterprise GenAI spending 2025 | $37B | Menlo Ventures^[5]^ |
| Enterprises with agents in production | 5-12% | Cleanlab, Recon Analytics^[8]^ |
| AI projects failing | 95% | MIT^[4]^ |
| Organizations citing tool sprawl | 70% | Zapier Survey^[8]^ |
| Observability acquisitions (18 months) | 7+ | Red Team analysis^[8]^ |
| AI startup acquisitions H1 2025 | 427 | LowTouch AI^[8]^ |
| Founders report mental health concerns | 72% | Freeman/Berkeley^[9]^ |
| Optimal founder age for success | 45 years | MIT (n=2.7M)^[9]^ |
| Market timing % of success variance | 42% | Bill Gross (200 companies)^[9]^ |
| Startup failures citing "no market need" | 42% | CB Insights (483 post-mortems)^[9]^ |
| VC-backed companies never return cash | 75% | HBS^[9]^ |
| AI share of all VC dollars 2025 | 52% (~$270B) | CB Insights, PitchBook^[5]^ |

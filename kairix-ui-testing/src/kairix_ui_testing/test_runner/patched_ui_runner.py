...
@click.option('--scenario', type=click.Choice([
    'default', 'streaming', 'long-output', 'error', 'timeout', 'empty', 'mixed'
]), default='default', help='Test scenario to use')
...
def run_patched_ui(scenario: str, port: int):
    ...
    # Apply all patches
    with mock.patch.multiple('kairix_offline.processing', **{
        ...
        from kairix_offline.ui.memory_pipeline import main
        
        print(f"🚀 Launching patched UI on port {port}")
        
        # Patch the launch to use our port
        from kairix_offline.ui import memory_pipeline

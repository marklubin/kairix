...
class TestImportTabStreaming(MemoryPipelineUITest):
    ...
    def test_import_streaming_display(self, ui_page: Page):
        ...
        text_snapshots = []
        for i in range(10):  # Increased iterations
            time.sleep(0.3)
            ...
            # Break early if we see completion
        ...
        assert "finished!" in final_text, "Should see completion message at end"
        
        # Verify scrollbar appears
        scroll_info = self.get_scroll_position(ui_page, self.SELECTORS["import_output"])
        # May not have scrollbar with only 5 conversations

...
from cognition_engine.persona.conversational import ConversationalPersona
from cognition_engine.types import Stimulus, Perception, StimulusType
from cognition_engine.utils import MessageTurnFormatter
...
def patch_isinstance_for_response_events(func):
    ...
    async def wrapper(*args, **kwargs):
        with patch('cognition_engine.persona.conversational.isinstance') as mock_isinstance:
...
def conversational_persona(mock_runner, mock_agent, mock_perceptors, mock_reflection_perceptors):
    ...
    persona_logger = logging.getLogger('cognition_engine.persona.conversational')
...
class TestConversationalPersona:
    ...
    def test_initialization(self, conversational_persona, mock_runner, mock_agent, mock_perceptors, mock_reflection_perceptors):
    ...
    @pytest.mark.asyncio
    @patch_isinstance_for_response_events
    async def test_react_with_user_message(self, conversational_persona, mock_runner, mock_perceptors, mock_reflection_perceptors):
    ...
    @pytest.mark.asyncio
    @patch_isinstance_for_response_events
    async def test_logging_behavior(self, conversational_persona, mock_runner, caplog):
        ...
        with caplog.at_level(logging.INFO):
            ...
            assert "Responding to stimulus of type: StimulusType.user_message" in caplog.text
            assert "Stimulus was user message responding by conversing" in caplog.text
    ...
    async def test_stream_event_filtering(self, conversational_persona, mock_runner):
        ...
        mock_stream = Mock()
        mock_stream.stream_events = Mock(return_value=self._async_generator([
            mock_event1, mock_event2, mock_event3, mock_event4
        ]))

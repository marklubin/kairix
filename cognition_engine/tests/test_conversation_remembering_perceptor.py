...
class TestConversationRememberingPerceptor:
    ...
    @pytest.mark.asyncio
    async def test_perceive_with_user_message(
        self, perceptor, mock_runner, mock_memory_provider
    ):
        """Test perceive method with a user message stimulus"""
        stimulus = Stimulus(
            content="Hello, how are you?", type=StimulusType.user_message
        )
    ...
    async def test_perceive_with_non_user_message(self, perceptor):
        ...
        stimulus = Stimulus(
            content="execution data", type=StimulusType.execution_attempt
        )
        perceptions = await perceptor.perceive(stimulus)
        assert perceptions == []
    ...
    async def test_logging_behavior(self, mock_logger, perceptor):
        ...
        mock_logger.info.assert_any_call(
            "Running paralleized insight generation agents..."
        )
    ...
    def test_agent_names_and_instructions(self, perceptor):
        ...
        assert (
            "embedding vector database" in perceptor.query_generating_agent.instructions
        )
    ...
    async def test_multiple_memories_processed(self, mock_runner):
        ...
        stimulus = Stimulus(
            content="What did we discuss?", type=StimulusType.user_message
        )

...
def main():
    ...
    scheduler_type = Prompt.ask(
        "Choose scheduler", choices=["inline", "hesitating"], default="inline"
    )
    ...
    # Create persona
    persona = Persona(perceptors=perceptors, proposers=proposers, schedulers=schedulers)

    # Subscribe persona to stimulus bus for self-perception
    stimulus_bus.subscribe(
        lambda s: persona.react(s) if s.type == StimulusType.execution_attempt else None
    )

    console.print(
        "[green]System initialized. Type messages to interact (or 'quit' to exit)[/green]"
    )
    ...
    while True:
        ...
        stimulus = Stimulus(
            content=user_input, type=StimulusType.user_message
        )

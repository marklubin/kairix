...
from ..types import Stimulus, Perception, StimulusType
from ..utils import MessageTurnFormatter
...
class ConversationalPersona:
    ...
    async def _converse(self, stimulus: Stimulus, perceptions: list[Perception]) -> AsyncIterator[str]:
        ...
        message = prompts.conversationalist_message_template_v2(
            perceptions, stimulus.content
        )

        logger.debug("Prepared message: %s", message)
        i = 0

        async for event in self.runner.run_streamed(
            self.actuating_agent, message
        ).stream_events():
    ...
    async def react(self, stimulus: Stimulus) -> AsyncIterator[str]:
        ...
        if stimulus.type == StimulusType.user_message:
            logger.info("Stimulus was user message responding by conversing.")

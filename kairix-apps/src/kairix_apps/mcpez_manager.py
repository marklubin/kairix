...
class MCPEzManager:
    ...
    def _build_image(self) -> None:
        ...
        logger.info(f"Checking for existing image: {MCPEZ_IMAGE}")
        exitcode, stdout, stderr = self._run_command([
            self.runtime, "images", "-q", MCPEZ_IMAGE
        ])
        logger.info(f"First check - exitcode: {exitcode}, stdout: [{stdout.strip()}], stderr: [{stderr[:100]}]")

        # Also check for localhost-prefixed name (Podman adds this prefix)
        if exitcode == 0 and not stdout.strip():
            logger.info(f"First check empty, trying localhost-prefixed: localhost/{MCPEZ_IMAGE}")
            exitcode2, stdout2, stderr2 = self._run_command([
                self.runtime, "images", "-q", f"localhost/{MCPEZ_IMAGE}"
            ])
            logger.info(f"Localhost check - exitcode: {exitcode2}, stdout: [{stdout2.strip()}]")

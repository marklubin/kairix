...
export class ContextService {
  ...
  private constructor() {
    ...
    this.apiKey = localStorage.getItem('kairix-auth-key') || ''
  ...
  updateApiKey(apiKey: string): void {
    this.apiKey = apiKey
    console.log('ContextService API key updated:', {
      hasApiKey: !!apiKey,
  ...
  async sendContextUpdate(update: ContextUpdate): Promise<void> {
    try {
      // Check for the latest API key from localStorage
      const currentApiKey = localStorage.getItem('kairix-auth-key') || this.apiKey
      if (currentApiKey !== this.apiKey) {
        this.apiKey = currentApiKey
        console.log('Updated API key from localStorage')
      }
      ...
      // Use x-api-key header (lowercase) as expected by the server
      if (currentApiKey) {
        headers['x-api-key'] = currentApiKey
        console.log('Sending with API key:', currentApiKey.substring(0, 8) + '...')
      } else {
        console.warn('No API key available for context update!')
      }

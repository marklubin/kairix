...
from kairix_core.tts.elevenlabs_tts import ElevenLabsTTS
...
class TestElevenLabsTTS:
    ...
    @patch("kairix_core.tts.elevenlabs_tts.ElevenLabs")
    ...
    @patch("kairix_core.tts.elevenlabs_tts.ElevenLabs")
    def test_tts_sync_with_custom_voice(self, mock_elevenlabs_class, tts_instance, sample_audio_bytes):
        ...
        sample_rate, audio_array = tts_instance.tts("Hello, world!", voice_id="custom-voice")
    ...
    @patch("kairix_core.tts.elevenlabs_tts.ElevenLabs")
    def test_stream_tts_sync(self, mock_elevenlabs_class, tts_instance, sample_audio_bytes):
        ...
        chunks = [sample_audio_bytes[i : i + chunk_size] for i in range(0, len(sample_audio_bytes), chunk_size)]
    ...
    @pytest.mark.asyncio
    @patch("kairix_core.tts.elevenlabs_tts.AsyncElevenLabs")
    async def test_atts_async(self, mock_async_elevenlabs_class, tts_instance, sample_audio_bytes):
    ...
    @pytest.mark.asyncio
    @patch("kairix_core.tts.elevenlabs_tts.AsyncElevenLabs")
    async def test_stream_tts_async(self, mock_async_elevenlabs_class, tts_instance, sample_audio_bytes):
        ...
        results = []
        async for sample_rate, audio_chunk in tts_instance.stream_tts("Hello, async streaming!"):
    ...
    @pytest.mark.asyncio
    @patch("kairix_core.tts.elevenlabs_tts.AsyncElevenLabs")
    async def test_stream_tts_async_error_handling(self, mock_async_elevenlabs_class, tts_instance):
        ...
        mock_async_text_to_speech.stream = Mock(side_effect=Exception("Async API Error"))
        mock_async_client.text_to_speech = mock_async_text_to_speech
    ...
    @patch("kairix_core.tts.elevenlabs_tts.ElevenLabs")
    def test_stream_with_small_chunks(self, mock_elevenlabs_class, tts_instance, sample_audio_bytes):
    ...
    @patch("kairix_core.tts.elevenlabs_tts.logger")
    @patch("kairix_core.tts.elevenlabs_tts.ElevenLabs")
    def test_chunk_conversion_error_handling(self, mock_elevenlabs_class, mock_logger, tts_instance):
        ...
        # Properly mock the text_to_speech attribute
        mock_text_to_speech = Mock()
        mock_text_to_speech.stream = Mock(return_value=iter([invalid_chunk, valid_chunk]))
        mock_client.text_to_speech = mock_text_to_speech
        tts_instance.client = mock_client
